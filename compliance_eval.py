"""Núcleo del eval de loan-scoring — AGNÓSTICO al framework MLOps (no importa DVC/MLflow).
Carga el dato vía Croissant (§2), corre la SDK venturalitica (vl.monitor abre la sesión +
los probes —incl. BOMProbe—; vl.enforce evalúa el OSCAL en dos fases Art.10/Art.15), PROMUEVE
el bom.json del run a .venturalitica/bom.json (ruta que lee el motor) y vuelca metrics.json
plano {control_id: {value, power}}. NO juzga: el veredicto autoritativo lo pone el motor Rust
contra el MISMO OSCAL."""
...
def run(build_model, df: pd.DataFrame | None = None, oscal_path: str = OSCAL):
    ...
    with contextlib.redirect_stdout(sys.stderr):
        with vl.monitor(name="loan-scoring", label="venth eval"):
